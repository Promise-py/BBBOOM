...
void Battery_Init(BATTERY *battery,float alarm_voltage,float cut_off_voltage)
{
    ...
    for ( i = 0; i < 4; i++)
    {
        battery[0].Cells_Voltage[i] = (float)ADC1_value[2+i]/4095*3.3*Resistance;
    }
    battery[0].Cells_Voltage[4] = (float)ADC2_value[0]/4095*3.3*Resistance;
    battery[0].Cells_Voltage[5] = (float)ADC2_value[1]/4095*3.3*Resistance;
    

    //二号电池电芯电压录入
    for ( i = 0; i < 6; i++)
    {
        battery[1].Cells_Voltage[i] = (float)ADC2_value[2+i]/4095*3.3*Resistance;
    }

    //三号电池电芯电压录入
    battery[2].Cells_Voltage[0] = (float)ADC1_value[6]/4095*3.3*Resistance;
    battery[2].Cells_Voltage[1] = (float)ADC1_value[7]/4095*3.3*Resistance;
    battery[2].Cells_Voltage[2] = (float)ADC1_value[0]/4095*3.3*Resistance;
    battery[2].Cells_Voltage[3] = (float)ADC1_value[1]/4095*3.3*Resistance;
    ...
    for ( i = 0; i < 6; i++)
    {
        ...
        {
            battery[2].Battery_Voltage += battery[2].Cells_Voltage[2];
            battery[2].Cells_Num++;
        }
    }
}
